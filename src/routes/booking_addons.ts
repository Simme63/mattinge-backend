...
BookingAddonRoutes.get("/:id", async (context) => {
    try {
        ...
        return context.json(
            { message: "Booking Addon Found Successfully", bookingAddon: bookingAddon[0] },
            200
        );
    } catch (error: unknown) {
...
BookingAddonRoutes.post("/", async (context) => {
    try {
        ...
        return context.json(
            { message: "Booking Addon Created Successfully", bookingAddon: insertedBookingAddon[0] },
            201
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);
...
BookingAddonRoutes.patch("/:id", async (context) => {
    try {
        const id = context.req.param("id");
        const updatedBookingAddon = await context.req.json<Partial<IBookingAddon>>();
        const updated = await sql<IBookingAddon[]>`
            UPDATE booking_addons SET ${sql(updatedBookingAddon)} WHERE id = ${id} RETURNING *;
        `;
        ...
        return context.json(
            { message: "Booking Addon Updated Successfully", bookingAddon: updated[0] },
            200
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);
...
BookingAddonRoutes.delete("/:id", async (context) => {
    try {
        ...
        return context.json(
            { message: "Booking Addon Deleted Successfully", bookingAddon: deleted[0] },
            200
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);

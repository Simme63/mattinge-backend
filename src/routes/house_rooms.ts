...
HouseRoomsRoutes.get("/:id", async (context) => {
    try {
        ...
        return context.json(
            { message: "House room found successfully", houseRoom: houseRoom[0] },
            200
        );
    } catch (error: unknown) {
...
HouseRoomsRoutes.post("/", async (context) => {
    try {
        ...
        return context.json(
            { message: "House room created successfully", houseRoom: insertedHouseRoom[0] },
            201
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);
...
HouseRoomsRoutes.patch("/:id", async (context) => {
    try {
        const id = context.req.param("id");
        const updatedHouseRoom = await context.req.json<Partial<IHouse_Rooms>>();
        const updated = await sql<IHouse_Rooms[]>`
            UPDATE house_rooms SET ${sql(updatedHouseRoom)} WHERE id = ${id} RETURNING *;
        `;
        ...
        return context.json(
            { message: "House room updated successfully", houseRoom: updated[0] },
            200
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);
...
HouseRoomsRoutes.delete("/:id", async (context) => {
    try {
        ...
        return context.json(
            { message: "House room deleted successfully", houseRoom: deleted[0] },
            200
        );
    } catch (error: unknown) {
        return context.json({ message: String(error) }, 500);
